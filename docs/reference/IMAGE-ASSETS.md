...
Original source and migration notes are preserved in git history. The active
tree keeps only the deployable image set.
```
...
Compression pipeline used q=78 JPG plus WebP siblings and 768px mobile crops.
...
**Placement**: `<meta property="og:image">` in each page's `<head>`.
